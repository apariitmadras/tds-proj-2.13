...
def _import_fail(prefix: str, err: Exception):
    # Raise a clear message so Railway logs show exactly what failed
    import traceback
    tb = traceback.format_exc()
    raise RuntimeError(f"{prefix}: {err}\n{tb}") from err

# --- Wrapped imports (diagnostics) ---
try:
    from config import (
    ...
    _import_fail("Import error in config", e)

try:
    from llm_client import plan_task, generate_code, compose_answer
except Exception as e:
    _import_fail("Import error in llm_client", e)

try:
    from executor import run_user_code
except Exception as e:
    _import_fail("Import error in executor", e)

try:
    from format_handler import (
        make_format_spec, validate_and_coerce, ValidationError, make_dummy_answer
    )
except Exception as e:
    _import_fail("Import error in format_handler", e)

# ... the rest of your orchestrator.py code follows ...
